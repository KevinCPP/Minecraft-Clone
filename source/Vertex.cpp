...
namespace Geometry {
    ...
    void Vertex::setPosition(float x, float y, float z) {
        assert(VERTEX_SIZE >= 3 && "Vertex::setPosition, N < 3");
        data[0] = x; data[1] = y; data[2] = z;
    }

    void Vertex::setTextureCoords(float u, float v) {
        assert(VERTEX_SIZE >= 5 && "Vertex::setTextureCoords, N < 5");
        data[3] = u; data[4] = v;
    }

    void Vertex::setNormal(float x, float y, float z) {
        assert(VERTEX_SIZE >= 8 && "Vertex::setNormal, N < 8");
    ...
    glm::vec3 Vertex::getPosition() const {
        assert(VERTEX_SIZE >= 3 && "Vertex::getPosition, VERTEX_SIZE < 3");
        return glm::vec3(data[0], data[1], data[2]);
    }

    glm::vec2 Vertex::getTextureCoords() const {
        assert(VERTEX_SIZE >= 5 && "Vertex::getTextureCoords, VERTEX_SIZE < 5");
        return glm::vec2(data[3], data[4]);
    }

    glm::vec3 Vertex::getNormal() const { 
        assert(VERTEX_SIZE >= 8 && "vertex::getNormal, VERTEX_SIZE < 8"); 
